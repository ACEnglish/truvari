...
def flush_stack(in_vcf, stack, out_name):
    """
    ...
    ret = cmd_exe(f"bgzip -f {out_name}")
    if ret.ret_code != 0:
        logging.error(ret)
        exit()
    ret = cmd_exe(f"tabix -f {out_name}.gz")
    if ret.ret_code != 0:
        logging.error(ret)
        exit()
...
def divide_main(args):
    ...
    for entry in in_vcf:
        if entry.chrom != stack[0].chrom and len(stack) >= args.min:
            flush_stack(in_vcf, stack, out_name_template.format(part_num=len(cluster_counts)))
