...
def make_giabreport(args, stats_box):
    ...
    sz_type = pd.CategoricalDtype(categories=["50to99", "100to299", "300to999", "gt1000"], ordered=True)
    ...
    with open(os.path.join(args.output, "giab_report.txt"), 'w') as sum_out:
        ...
        view = data[base_calls].groupby(["sizecat", "SVTYPE", "state"]).size().unstack(fill_value=0)
        view["pct"] = view["tpbase"] / view.sum(axis=1)
        sum_out.write(view.to_csv(sep="\t") + '\n')
        sum_out.write("\n")

        sum_out.write("# State by REPTYPE\n")
        view = data[base_calls].groupby(["REPTYPE", "state"]).size().unstack(fill_value=0)
        ...
        view = data[base_calls].groupby(["tech_str", "state"]).size().unstack(fill_value=0).sort_index()
        view["pct"] = view["tpbase"] / view.sum(axis=1)
        sum_out.write(view.to_csv(sep="\t") + '\n')
        sum_out.write("\n")
        ...
        view.loc["recall"] = view.loc["tpbase"] / (view.loc["tpbase"] + view.loc["fn"])
        view.loc["precision"] = view.loc["tp"] / (view.loc["tp"] + view.loc["fp"])
