...
if [ $test_vcf2df_single ]; then
    assert_exit_code 0
    df_check $ANSDIR/vcf2df/single_vcf2df.jl $OD/single_vcf2df.jl
fi
...
if [ $test_vcf2df_subset ]; then
    assert_exit_code 0
    df_check $ANSDIR/vcf2df/subset_vcf2df.jl $OD/subset_vcf2df.jl
fi

run test_vcf2df_multi $truv vcf2df -f -i $INDIR/variants/multi.vcf.gz $OD/multi_vcf2df.jl
if [ $test_vcf2df_multi ]; then
    assert_exit_code 0
    df_check $ANSDIR/vcf2df/multi_vcf2df.jl $OD/multi_vcf2df.jl
fi
