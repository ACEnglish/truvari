...
if [ $test_vcf2df_bare ]; then
    assert_exit_code 0
    df_check test_vcf2df_bare_result $ANSDIR/vcf2df/vcf2df_bare.jl $OD/vcf2df_bare.jl
fi

run test_vcf2df $truv vcf2df -f -i -n $INDIR/variants/input1.vcf.gz $OD/vcf2df.jl
if [ $test_vcf2df ]; then
    assert_exit_code 0
    df_check test_vcf2df_result $ANSDIR/vcf2df/vcf2df.jl $OD/vcf2df.jl
fi

run test_vcf2df_dir $truv vcf2df -f -i -b -n $OD/bench23/ $OD/truv2df.jl
if [ $test_vcf2df_dir ]; then
    assert_exit_code 0
    df_check test_vcf2df_dir_result $ANSDIR/vcf2df/truv2df.jl $OD/truv2df.jl
fi
